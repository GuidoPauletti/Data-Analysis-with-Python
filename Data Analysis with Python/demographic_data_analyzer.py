...
def calculate_demographic_data(print_data=True):
    ...
    max_rich_country_percentage = 0.0
    highest_earning_country = None

    for country in df['native-country'].value_counts().index:
        current_country_people = df[df['native-country'] == country]
        current_country_rich = np.float64(100 * len(current_country_people[current_country_people['salary'] == '>50K']) / current_country_people.shape[0]).round(1)
